 ...
 * 懒汉式获取一个单例的
 */
public class ProducerLazy {

    private static ProducerLazy instance;

    private KafkaProducer<String, String> kafkaProducer;

    private ProducerLazy() {
     ...
     * @return
     */
    public static synchronized ProducerLazy getInstance() {
            if(instance == null) {
                instance = new ProducerLazy();
            }
            return instance;
    }

    public Future<ProducerMetadata> send(String value, Callback callback){
        /**
         * key  是null。轮询方式
         */
        Future send = kafkaProducer.send(new ProducerRecord<String,String>(KafkaProducerExample.topicNew,null,value),callback);
        return send;
    }
