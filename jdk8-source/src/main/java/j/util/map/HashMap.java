...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    public void printTableSize() {
       int size =  table == null ? 0 : table.length;
       printLog("桶的数组长度为:"+size);
    }
...
//        printLog("调用 putVal，key = " + key + "，value = " + value);
    ...
    final Node<K,V>[] resize() {
        ...
        printLog("开始扩容，旧数组长度为: " + (oldTab == null ? 0 : oldTab.length));
        ...
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {
                printLog("达到最大容量，停止扩容");
                threshold = Integer.MAX_VALUE;
                return oldTab;
            } else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                    oldCap >= DEFAULT_INITIAL_CAPACITY) {
                newThr = oldThr << 1; // double threshold
                printLog("扩容一倍，新容量: " + newCap + "，新阈值: " + newThr);
            }
        } else if (oldThr > 0) { // initial capacity was placed in threshold
            newCap = oldThr;
            printLog("使用旧阈值作为新容量: " + newCap);
        } else { // zero initial threshold signifies using defaults
            newCap = DEFAULT_INITIAL_CAPACITY;
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
            printLog("使用默认容量和负载因子，新容量: " + newCap + "，新阈值: " + newThr);
        }
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null) {
                        printLog("单节点，直接迁移到新数组索引: " + (e.hash & (newCap - 1)));
                        newTab[e.hash & (newCap - 1)] = e;
                    } else if (e instanceof TreeNode) {
                        printLog("树节点，调用 split 进行迁移");
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                    } else { // preserve order
                        printLog("链表节点，进行拆分迁移");
                        ...
                        if (loTail != null) {
                            loTail.next = null;
                            newTab[j] = loHead;
                            printLog("低位链表迁移到新数组索引: " + j);
                        }
                        if (hiTail != null) {
                            hiTail.next = null;
                            newTab[j + oldCap] = hiHead;
                            printLog("高位链表迁移到新数组索引: " + (j + oldCap));
                        }
                    }
                }
            }
        ...
        printLog("扩容结束，新数组长度: " + newCap + "，新阈值: " + newThr);
        return newTab;
    }
