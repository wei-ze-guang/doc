...
//        }
    ...
    class OneInnerThread extends Thread {
        ...
        public void run() {
            ...
            // 虚假唤醒（Spurious Wakeup）”是指一个正在执行 wait() 或 await() 的线程，在没有收到 notify() 或 signal() 通知、也没有条件满足的情况下被唤醒了。
            //这里的话如果在调用wait之前，上面的同步方法已经执行完了，所以下面要加一个条件
            while (true) {   // 用循环判断条件，避免“虚假唤醒”和“错过通知”，这个这个条件就是我们想要的，如果不是这个条件线程被虚假唤醒，在wait一次
                try {
